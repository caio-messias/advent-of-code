...
fn max_signal(tape: &Vec<i64>) -> i32 {
    ...
    for phase in phases {
        ...
        let mut amp_b = IntcodeMachine::new(tape.clone())
            .with_input(phase[1])
            .with_input(amp_a.get_output().unwrap());
        amp_b.run();

        let mut amp_c = IntcodeMachine::new(tape.clone())
            .with_input(phase[2])
            .with_input(amp_b.get_output().unwrap());
        amp_c.run();

        let mut amp_d = IntcodeMachine::new(tape.clone())
            .with_input(phase[3])
            .with_input(amp_c.get_output().unwrap());
        amp_d.run();

        let mut amp_e = IntcodeMachine::new(tape.clone())
            .with_input(phase[4])
            .with_input(amp_d.get_output().unwrap());
        amp_e.run();

        largest_output_signal = max(largest_output_signal, amp_e.get_output().unwrap());
    }

    return largest_output_signal as i32
}

fn max_signal_with_feedback(tape: &Vec<i64>) -> i32 {
    let mut largest_output_signal = 0;
    ...
    for phase in phases {
        ...
        loop {
            ...
            amp_b.add_input(amp_a.get_output().unwrap());
            amp_b.run();

            amp_c.add_input(amp_b.get_output().unwrap());
            amp_c.run();

            amp_d.add_input(amp_c.get_output().unwrap());
            amp_d.run();

            amp_e.add_input(amp_d.get_output().unwrap());
            amp_e.run();
            output_end = amp_e.get_output().unwrap();
