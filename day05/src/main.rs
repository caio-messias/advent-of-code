...
fn parse_mode(i: isize) -> ParameterMode {
    match i {
...
impl IntcodeMachine {
    fn new(tape: Vec<isize>) -> IntcodeMachine {
        IntcodeMachine {tape, position: 0, input: Vec::new(), output: Vec::new()}
    }

    fn init(mut tape: Vec<isize>, noun: isize, verb: isize) -> IntcodeMachine {
        tape[1] = noun;
        tape[2] = verb;
    ...
    fn fetch1mode(&mut self) -> ParameterMode {
        ...
        parse_mode(parameter_mode % 10)
    }

    fn fetch2mode(&mut self) -> (ParameterMode, ParameterMode) {
        let mode1 = self.fetch1mode();
        let mode2 = self.tape[self.position] / 1000;

        (parse_mode(mode2 % 10), mode1)
    }

    fn fetch3mode(&mut self) -> (ParameterMode, ParameterMode, ParameterMode) {
        let (mode2, mode1) = self.fetch2mode();
        let mode3 = self.tape[self.position] / 10000;

        (parse_mode(mode3 % 10), mode2, mode1)
    }
    ...
    fn addi(&mut self) {
        let (_mode3, mode2, mode1) = self.fetch3mode();
    ...
    fn run_for_target(&mut self, target: usize) -> isize {
        loop {
